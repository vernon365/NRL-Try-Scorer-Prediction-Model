...
from keras.src.layers import Input, Dense, Embedding, Flatten, Concatenate, Dropout
...
# Encode 'Player' column with LabelEncoder
player_encoder = LabelEncoder()
X['Player'] = player_encoder.fit_transform(X['Player'])
...
X_combined = np.hstack([X[['Player']], X_encoded, X_scaled])
...
player_embedding = Embedding(input_dim=len(player_encoder.classes_), output_dim=10, name='Player_Embedding')(player_input)
player_flatten = Flatten(name='Player_Flatten')(player_embedding)

other_input = Input(shape=(X_encoded.shape[1] + X_scaled.shape[1],), name='Other_Input')

# Concatenate embeddings and other inputs
concatenated = Concatenate(name='Concatenate')([player_flatten, other_input])
...
# Define the model
model = Model(inputs=[player_input, other_input], outputs=output)
